#!/usr/bin/env bash
# =============================================================
# Xray Lite Ultimate 一键部署脚本 (Final Version)
# 特性：Reality / TUIC v5 / TLS Fallback / WS / gRPC / 订阅链接
# 优化：Reality 和 VLESS+TLS 端口合并到 443
# =============================================================
set -e
...
WEB_PORT=12789  # 轻量 Web 管理页端口，通过 443 Fallback 访问
...
# ------------------------------------
install_base() {
  log "安装基础依赖：curl, wget, unzip, cron, socat, jq, python3, qrencode..."
...
issue_cert() {
  ...
  log "申请证书..."
  # 使用 --issue --standalone 模式
  $ACME_HOME/acme.sh --issue --standalone -d "$DOMAIN" --keylength 2048 --log || err "证书申请失败！请检查域名解析和 80 端口占用。"
  ...
  cat > "$CONF" <<EOF
{
  ...
  "inbounds": [
    ...
    // ===== VLESS + TLS + fallback（监听 443，实现端口复用）=====
    {
      "port": $PORT_TLS,
      "protocol": "vless",
      "settings": {
      ...
      "streamSettings": {
        ...
        "tlsSettings": {
          ...
          "fallbacks": [
            { "dest": 80 }, // 非TLS/SNI 命中 - 导向假网站 (需手动配置 Nginx 监听 80)
            { "alpn": "h2", "dest": $WEB_PORT, "xver": 1 } // HTTP/2 命中 - 导向 Web 管理页
          ]
        }
      }
    },
...
EOF
  ...
  # Reality 链接
  VLESS_REALITY_LINK="vless://${UUID}@${SERVER_IP}:${PORT_REALITY}?encryption=none&security=reality&sni=${REALITY_SNI}&fp=chrome&pbk=${REALITY_PUBLIC}&sid=${REALITY_SID}&type=tcp#REALITY-${REALITY_SNI}"
  
  # TLS Vision 链接
  VLESS_TLS_LINK="vless://${UUID}@${DOMAIN}:${PORT_TLS}?encryption=none&security=tls&sni=${DOMAIN}&fp=chrome&type=tcp#VLESS-TLS-${DOMAIN}"
  
  # WS 链接
  VLESS_WS_LINK="vless://${UUID}@${DOMAIN}:${PORT_WS}?encryption=none&security=tls&type=ws&path=${WS_PATH}&host=${DOMAIN}&sni=${DOMAIN}#VLESS-WS-${DOMAIN}"
  
  # gRPC 链接
  VLESS_GRPC_LINK="vless://${UUID}@${DOMAIN}:${PORT_GRPC}?encryption=none&security=tls&type=grpc&serviceName=${GRPC_SVC}&sni=${DOMAIN}#VLESS-gRPC-${DOMAIN}"
  
  # TUIC v5 链接
  TUIC_LINK="tuic://${UUID}:${TUIC_PASS}@${DOMAIN}:${PORT_TUIC}?alpn=h3&congestion_control=bbr&zero_rtt=true#TUIC-v5-${DOMAIN}"
...
--- Reality (主力/免证书) ---
端口: $PORT_REALITY (443 TCP)
SNI: $REALITY_SNI
PublicKey: $REALITY_PUBLIC
ShortID: $REALITY_SID

--- VLESS + TLS (Fallback 掩护) ---
端口: $PORT_TLS (443 TCP)
Web管理页端口 (Fallback 目标): $WEB_PORT
...
EOF
  ...
  # 清除旧的规则 (可选)
  iptables -D INPUT -p tcp --dport 443 -m string --string "GET /" --algo bm -j DROP 2>/dev/null || true
  iptables -D INPUT -p udp --dport $PORT_TUIC -j ACCEPT 2>/dev/null || true
  
  # 1. 阻断 HTTP 探测 443 端口 (针对 TLS 和 Reality)
  # 任何试图在 443 端口使用明文 HTTP GET 探测的流量，直接丢弃
  iptables -A INPUT -p tcp --dport 443 -m string --string "GET /" --algo bm -j DROP
...
<p>管理页端口: ${WEB_PORT} (请勿暴露给外网，通过 Fallback/alpn=h2 访问)</p>
<pre id="info"></pre>
<script>
...
try:
    HTTPServer(('127.0.0.1',$WEB_PORT),H).serve_forever()
...
EOF
  ...
  # 注意：Python Server 仅监听 127.0.0.1，防止 Web 管理页端口直接暴露给公网
...
uninstall() {
  ...
  warn "iptables -D INPUT -p tcp --dport 443 -m string --string \"GET /\" --algo bm -j DROP"
